...
#define DRV_NAME "rockchip-snd-wm8960"

static struct snd_soc_jack mach_wm8960_jack;
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	int ret = 0;
        ...
        mclk = rate * div_bck_over_lrck * div_mclk_over_bck;
...
static int snd_rk_mc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
        ...
        ret = snd_soc_of_parse_audio_simple_widgets(card, "rockchip,audio-widgets");
        if (ret) {
                dev_err(&pdev->dev,
...
static struct platform_driver snd_rk_wm8960_driver = {
...
module_platform_driver(snd_rk_wm8960_driver);

MODULE_AUTHOR("Xing Zheng <zhengxing@rock-chips.com>");
MODULE_DESCRIPTION("Rockchip wm8960 machine ASoC driver");
